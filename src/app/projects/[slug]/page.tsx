...
export default async function ProjectPage({
  ...
  return (
    <main className="min-h-screen bg-background text-slate-900 dark:bg-slate-950 dark:text-slate-50 flex flex-col md:flex-row">
      ...
      <section className="flex-1 w-full px-6 pb-10 md:px-16 md:py-14">
          ...
          {project.videos && project.videos.length > 0 && (
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {project.videos.map((src) => (
                <div
                  key={src}
                  className="relative overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 aspect-video"
                >
                  <video
                    src={src}
                    controls
